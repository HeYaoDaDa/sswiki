...
def generate_ship(ship_data_csv, ship_json, ship_systems_csv):
    ...
    if is_phasecloak:
        shield_arc = util.my_round(ship_data_csv["max flux"]*ship_data_csv["phase cost"])
    elif have_shield:
    ...
    if is_phasecloak:
        shield_upkeep = util.my_round(ship_data_csv["max flux"]*ship_data_csv["phase upkeep"])
    elif have_shield:
        shield_upkeep = util.my_round(ship_data_csv["flux dissipation"]*ship_data_csv["shield upkeep"])
    else:
        shield_upkeep = ''
    ...
    if not pd.isna(system_id):
        system_name = ship_systems_csv.loc[ship_systems_csv['id'] == system_id, 'name'].iloc[0]
        system_description = ship_systems_csv.loc[ship_systems_csv['id'] == system_id, 'text3'].iloc[0]

    if not pd.isna(special_system_id):
        special_system_name = ship_systems_csv.loc[ship_systems_csv['id'] == special_system_id, 'name'].iloc[0]
        special_system_description = ship_systems_csv.loc[ship_systems_csv['id'] == special_system_id, 'text3'].iloc[0]

    weapon_slot_map = {}
    if ship_json.get('weaponSlots') is not None:
        for weapon_slot in ship_json["weaponSlots"]:
            if weapon_slot['type'] in ['BALLISTIC','ENERGY','MISSILE','HYBRID','SYNERGY','COMPOSITE','UNIVERSAL']:
    ...
    if len(weapon_slot_map) > 0:
        ...
        for type,map in weapon_slot_map.items():
            for size,num in map.items():
                installation_slot_list.append(f"{num}x {slot_size_map[size]}{slot_type_map[type]}")
